(() => {
  ...
  function Mesh(id, riven, rect, children, entry, exit) {
    ...
    if (riven.network[entry]) {
      this.ports.entry.connect(riven.selector(entry).ports.input);
    }
    if (riven.network[exit]) {
      riven.selector(exit).ports.output.connect(this.ports.exit);
    }
  ...
  var RIVEN = new riven_default();
  var \u00D8 = RIVEN.selector.bind(RIVEN);
